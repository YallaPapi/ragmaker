...
class RAGMakerDesktopApp {
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        this.httpServer.listen(0, (error) => {
          ...
          this.backendPort = this.httpServer.address().port;
          console.log(`🚀 RAGMaker Desktop backend running on port ${this.backendPort}`);
          resolve();
        });
      });
  ...
  createMainWindow() {
    this.mainWindow = new BrowserWindow({
      ...
      minWidth: 800,
      minHeight: 600,
      icon: path.join(__dirname, '../app/frontend/assets/icon.png'),
      webPreferences: {
    ...
    // Load the frontend using the dynamic port
